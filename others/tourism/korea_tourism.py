...
def getData(table, year, month):
    ...
    if not os.path.exists(f'excel/{year}'):
        os.mkdir(f'excel/{year}')

    dDf.to_excel(f'excel/{year}/{year}-{month}.xlsx', index=False)
...
if __name__ == '__main__':
    ...
    df.to_excel(f'excel/{YYYY}_2020-{MM}.xlsx', index=False)
